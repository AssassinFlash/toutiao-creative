      ...
      <a-card hoverable class="filter-card">
        ...
        <template #title>
          <a-breadcrumb separator=">">
            ...
            <a-breadcrumb-item>
              <router-link :to="{name:'Article'}">内容管理</router-link>
            </a-breadcrumb-item>
          </a-breadcrumb>
        </template>
        ...
        <a-form :label-col="labelCol" :wrapper-col="wrapperCol">
          ...
          <a-form-item label="频道" style="width:400px">
            <a-select
              placeholder="请选择频道"
      ...
      <a-card hoverable>
        ...
        >
          ...
          <template #image="{ record }">
            <img
              :src="record.cover.images[0]"
              alt=""
              class="article-image"
              v-show="record.cover.images[0]"
            >
            <img
              src="@/assets/superman.svg" alt=""
              v-show="!record.cover.images[0]"
              class="article-image"
            >
          </template>
          ...
          <template #status="{ record }">
            <a-tag
              :color="articlesStatus[record.status].type"
            >
  ...
  setup () {
    ...
    const onConfirmDelete = async (articleID, articleTitle) => {
      try {
        ...
        console.log(resDelete.status, typeof resDelete.status)
